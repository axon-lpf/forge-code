package com.forgecode.plugin.idea.agent
...
""".trimIndent()
    ...
    /** 写入文件（通过 CodeApplyManager 弹出 Diff 视图） */
    fun writeFile(project: Project, relativePath: String, content: String): ToolResult {
        ...
        return try {
            ...
            ApplicationManager.getApplication().invokeLater {
                com.forgecode.plugin.idea.util.CodeApplyManager.applyCode(
                    project, content, lang, fileName
                )
            }
            ToolResult("write_file", true, "✅ 已弹出 Diff 视图，等待用户确认写入: $relativePath")
        } catch (e: Exception) {
