    ...
    override fun apply() {
        ...
        if (settings.activeProvider.isBlank()) {
            ...
            if (firstConfigured != null) {
                ...
                val meta = com.forgecode.plugin.llm.provider.ProviderRegistry.get(firstConfigured.key)
...
class ForgeSettingsComponent {
    ...
    init {
        ...
        // 初始化行内补全开关状态
        inlineCompletionCheckbox.isSelected = ForgeSettings.getInstance().inlineCompletionEnabled
    ...
    fun loadProviderConfigs() {
        ...
        editingConfigs.putAll(ForgeSettings.getInstance().getProviderConfigMap())
    ...
    fun applyProviderConfigs() {
        ForgeSettings.getInstance().setProviderConfigMap(editingConfigs)
        configModified = false
    }
    ...
    private fun addCustomProvider() {
        ...
        if (dialog.showAndGet()) {
            ...
            if (r.apiKey.isNotBlank()) {
                ForgeSettings.getInstance().saveApiKey(ProviderRegistry.CUSTOM_PREFIX + r.id, r.apiKey)
            }
            configModified = true
            providerTableModel.fireTableDataChanged()
        }
...
class ProviderConfigDialog(
    ...
    fun getConfig(): ProviderManager.ProviderConfig {
        ...
        config.connectTimeout = ForgeSettings.getInstance().connectTimeout
        config.readTimeout = ForgeSettings.getInstance().readTimeout
        return config
    }
}
