...
object CodeApplyManager {
    ...
    private class CodeDiffDialog(
        ...
        init {
            title = "CodeForge — 代码变更预览：${targetFile.name}"
    ...
    private fun writeToFile(project: Project, virtualFile: VirtualFile, content: String) {
        WriteCommandAction.runWriteCommandAction(project, Runnable {
            try {
                ...
                ApplicationManager.getApplication().invokeLater {
                    Messages.showErrorDialog(project, "写入失败：${e.message}", "CodeForge")
                }
            }
        })
    }
    ...
    private fun applyAsNewFile(
        ...
        if (baseDir == null) {
            Messages.showErrorDialog(project, "无法确定目标目录", "CodeForge")
            return
        }
        ...
        WriteCommandAction.runWriteCommandAction(project, Runnable {
            try {
                ...
                log.error("创建文件失败", e)
                ApplicationManager.getApplication().invokeLater {
                    Messages.showErrorDialog(project, "创建失败：${e.message}", "CodeForge")
                }
            }
        })
    }
