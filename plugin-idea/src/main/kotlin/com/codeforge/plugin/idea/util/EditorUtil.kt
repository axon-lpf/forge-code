...
import com.codeforge.plugin.idea.toolwindow.CodeForgeChatPanel
...
object EditorUtil {
    ...
    /**
     * 向 CodeForge 聊天面板发送消息
     * （通过 ToolWindow 找到 CodeForgeChatPanel，调用 executeJS）
     */
    fun sendMessageToChat(project: Project, message: String) {
        val toolWindow = ToolWindowManager.getInstance(project).getToolWindow("CodeForge") ?: return
        val contentManager = toolWindow.contentManager
        val content = contentManager.getContent(0) ?: return
        val component = content.component

        // 找到 CodeForgeChatPanel（通过组件层次结构或直接引用）
        // 这里通过用户数据存储 panel 引用
        val panel = component.getClientProperty("CodeForgeChatPanel") as? CodeForgeChatPanel ?: return
