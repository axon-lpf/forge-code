 ...
 *
 * 对外接口保持简洁，供 CodeForgeChatPanel / StatusBar 等组件调用。
 */
...
class LlmService {
    ...
    fun chatStream(
        ...
        val provider = providerManager.activeProvider
        if (provider == null) {
            onError(IllegalStateException("未配置任何可用的模型提供商，请在 Settings → CodeForge 中配置 API Key"))
            return
        }
    ...
    private fun doNonStreamCall(
        provider: com.codeforge.plugin.llm.provider.LlmProvider,
        messages: List<Map<String, Any>>,
        onToken: (String) -> Unit,
        onDone: () -> Unit,
        onError: (Exception) -> Unit
     ...
     */
    private fun getNextAvailableProvider(excludeName: String): com.codeforge.plugin.llm.provider.LlmProvider? {
